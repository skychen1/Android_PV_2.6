...
//	mRaindropPaint.setStyle(Paint.Style.FILL);
   ...
   protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
	//设置宽高,默认200dp
//	int defaultSize = dp2px(getContext(), 300);
	int defaultSize = mAppContext.getResources().getDimensionPixelSize(R.dimen.x450);
   ...
   private void drawCircle(Canvas canvas, int cx, int cy, int radius) {
	...
	for (int i = 0; i < mCircleNum; i++) {
	   if (i==mCircleNum-1){
		...
		canvas.drawCircle(cx, cy, 5, mCirclePaint);
	   }else {
		canvas.drawCircle(cx, cy, radius - (radius / mCircleNum * i)-20, mCirclePaint);
	   }
